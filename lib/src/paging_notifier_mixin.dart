...
abstract interface class PagingNotifierMixin<D extends PagingData<T>, T> {
  ...
  Future<void> loadNext({Map<String, dynamic>? parameters});
  void forceRefresh();
}
...
abstract mixin class PagePagingNotifierMixin<T> implements PagingNotifierMixin<PagePagingData<T>, T> {
  ...
  @override
  Future<void> loadNext({Map<String, dynamic>? parameters}) async {
...
abstract mixin class OffsetPagingNotifierMixin<T> implements PagingNotifierMixin<OffsetPagingData<T>, T> {
  ...
  Future<void> loadNext({Map<String, dynamic>? parameters}) async {
    ...
    if (value.hasMore) {
      ...
      state = await state.guardPreservingPreviousOnError(
        () async {
          final next = await fetch(offset: value.offset, parameters: parameters);
